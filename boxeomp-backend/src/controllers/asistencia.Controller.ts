...
export const registrarAsistencia = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!dni || String(dni).trim() === '') {
      res.status(400).json({ message: 'Ingresá el DNI.' });
      return;
    }
    ...
    res.status(200).json({
      ...
      respondUnexpected(res, error, "registrar el ingreso");
  }
};
...
export const obtenerHistorialAsistencias = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(200).json({
      ...
      respondUnexpected(res, error, "cargar el historial de asistencias");
  }
};
...
export const obtenerMisAsistencias = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(200).json({
      ...
      respondUnexpected(res, error, "cargar tus asistencias");
  }
};
